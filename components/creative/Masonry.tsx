...
interface MasonryProps {
    ...
    onItemClick?: (item: MasonryItem, index: number) => void; // ✨ Added Handler
}
...
const Masonry = ({
    ...
    onItemClick // ✨ Destructure the callback
}: MasonryProps) => {
    ...
    const getInitialPosition = (item: any) => {
        ...
        switch (direction) {
            ...
            case 'bottom': return { x: item.x, y: typeof window !== 'undefined' ? window.innerHeight + 200 : 1000 };
            case 'left': return { x: -200, y: item.y };
            case 'right': return { x: typeof window !== 'undefined' ? window.innerWidth + 200 : 1000, y: item.y };
            case 'center':
                return {
    ...
    // Calculate container height based on grid to avoid collapse
    const containerHeight = useMemo(() => {
        if (grid.length === 0) return 800;
        return Math.max(...grid.map(i => i.y + i.h)) + 50;
    }, [grid]);
        ...
        <div ref={containerRef} className="list" style={{ height: containerHeight }}>
            {grid.map((item, index) => (
                <div
                    ...
                    onClick={() => onItemClick && onItemClick(item, index)} // ✨ Connect to Lightbox
